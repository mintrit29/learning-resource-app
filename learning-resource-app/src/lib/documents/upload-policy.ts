...
export const SUPPORTED_UPLOAD_EXTENSIONS = ["pdf", "pptx", "docx", "epub"] as const satisfies readonly SupportedExtension[];
export const SUPPORTED_UPLOAD_ACCEPT = SUPPORTED_UPLOAD_EXTENSIONS.map((extension) => `.${extension}`).join(",");
export const SUPPORTED_UPLOAD_LABEL = "PDF, PPTX, DOCX, EPUB";
