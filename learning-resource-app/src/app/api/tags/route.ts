...
export async function GET() {
  ...
  const tags = await db.tag.findMany({ where: { createdByUserId: session.user.id }, select: { id: true, name: true, normalizedName: true, description: true, isClassificationEnabled: true, createdAt: true, _count: { select: { aliases: true, documents: true } } }, orderBy: { name: "asc" } });
...
export async function POST(request: Request) {
  ...
  if (await findExactTagOrAlias(session.user.id, parsed.data.name)) return NextResponse.json({ message: "Tên môn học hoặc tên gọi khác này đã tồn tại" }, { status: 409 });
