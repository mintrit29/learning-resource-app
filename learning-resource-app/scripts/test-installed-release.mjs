...
try {
  ...
  const saved=await fetch(`${origin}/api/ai-providers`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({type:'CUSTOM',displayName:'Release QA Custom',baseUrl:'http://127.0.0.1:39999/v1',apiKey:'test-only',defaultChatModel:'test',isActive:true})});
  assert.equal(saved.status,201,await saved.text());
  await page.goto(`${origin}/settings/ai-providers`); await page.getByText('Release QA Custom',{exact:true}).first().waitFor();
  record('Custom API settings persisted');
