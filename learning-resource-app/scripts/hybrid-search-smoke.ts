...
try {
  ...
  documentIds.push(ownedDocument.id);
  const secondaryDocument = await db.document.create({
    data: {
  ...
  documentIds.push(secondaryDocument.id);
  ...
  const accentlessResults = await searchByKeyword("co so du lieu transaction", {}, 10);
  assert.ok(accentlessResults.length > 0, "Accentless Vietnamese query must find accented content");

  const filteredResults = await searchByKeyword("transaction", { fileType: "PPTX" }, 10);
  assert.equal(filteredResults.length, 0, "File type filter must be applied");
  const difficultyFilteredResults = await searchByKeyword("transaction", { difficulty: "ADVANCED" }, 10);
  assert.equal(difficultyFilteredResults.length, 0, "Difficulty filter must be applied");
  const topicFilteredResults = await searchByKeyword("transaction", { topic: "Cơ sở dữ liệu" }, 10);
  assert.equal(topicFilteredResults.length, 1, "Topic filter must be applied");

  const vectorResults = await searchByVector("transaction co so du lieu cho nguoi moi", {}, 10);
  assert.ok(vectorResults.length > 0, "Vector retrieval must find the embedded chunk");
  const hybridResults = await hybridSearch("transaction co so du lieu cho nguoi moi", {});
  ...
  console.log("PASS hybrid search: accent-insensitive keyword, semantic merge and topic/difficulty/file filters");
} finally {
